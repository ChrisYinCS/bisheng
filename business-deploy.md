...
2. 获取fingerprint
在宿主机器上，执行以下shell ，并获取 server_fingerprint.txt 到本机
```bash
#!/bin/bash
  ...
  cloud:
    gateway:
      ...
      routes:
        - id: bisheng-http
          # lb表示负载均衡
          uri: http://192.168.10.109:7861 #这里配置bisheng的http请求地址
          filters:
          ...
          predicates:
            - Path=/api/v1/**,/api/v2/**
        - id: bisheng-ws
          uri: ws://192.168.10.109:7861  #这里配置bisheng的websocket请求地址
          filters:
            - name: CacheRequestBody
              args:
                bodyClass: java.lang.String
...
5. 启动程序
docker-compose 方式 配置好之后，直接执行docker-compose up -d 即可启动
...
7. 修改 bisheng 前端 api 地址为 gateway 地址
举例：
...
8. 修改 bisheng 配置，开启商业版特性
目标是设置BISHENG_PRO=true的环境变量
- 如果 bisheng 是docker compose一键部署的，需要在docker compose文件中，将backend服务的环境变量配置增加BISHENG_PRO: true，示例如下：
```yml
services:
  backend:
    environment:
